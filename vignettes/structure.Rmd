...
* `new_a11ytable()` to create a data.frame object (with an additional 'a11ytable' S3 class) filled with all the information needed to create a spreadsheet output, as well as check the validity of the structure and provide errors or warnings
* ` create_a11y_wb()` to convert the output from `new_a11ytable()` to an [{openxlsx}](https://ycphs.github.io/openxlsx/) Workbook-class object, ready for writing to an xlsx file with `openxlsx::saveWorkbook()`
...
* `class.R` and `utils-class.R` contain code for handling the a11ytable class, most importantly the `new_a11ytable()` function
* `workbook.R`, `utils-workbook.R` and `utils-style.R` contain the code for creating a Workbook-class object with the `create_a11y_wb()` function
...
Basically, `new_a11ytable()` creates an object with classes data.frame and tbl (i.e. [tibble](https://tibble.tidyverse.org/)), along with an additional 'a11ytable' class.
...
Within `new_a11ytable()` itself are two major functions that help ensure proper construction of an a11ytable object:
...
The `create_a11y_wb()` function sets up an [{openxlsx}](https://ycphs.github.io/openxlsx/) Workbook-class object and fills it by iterating over a user-supplied the a11ytable-class object.
...
Several internal sub-functions within `create_a11y_wb()`---`.add_*()`, `.insert_()` and `.style_*()`---are responsible for adding sheets, inserting sheet elements and styling them.
