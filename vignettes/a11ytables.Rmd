...
1. Use `create_a11ytable()` to create a special dataframe object that contains all the information you need in your output
1. Use `generate_workbook()` to add styles and a workbook structure
...
Each argument to `create_a11ytable()` provides information required for each sheet of the spreadsheet.

You provide the majority of information as dataframes passed to the `tables` argument. It's preferable to prepare these first before passing them to `create_a11ytable()`.

Below are some demo tables that we can pass to `create_a11ytable()`. I've used `tibble::tribble()` for human-readable row-by-row dataframe construction, but you can just use `data.frame()`.
...
Now we can construct an a11ytable by passing the required sheet 'elements' as vectors (or a list in the case of `tables`) to the `create_a11ytable()` function.

```{r new-a11ytable}
library(a11ytables)
library(openxlsx)  # was installed with {a11ytables}

my_a11ytable <- create_a11ytable(
  tab_titles = c(
    "Cover",
    "Contents",
    "Notes",
...
Note that `create_a11ytable()` is the preferred method, but it's also possible to convert a correctly-formatted pre-built data.frame directly to an a11ytable with `as_a11ytable()`. You can try this using the `mtcars_df` data.frame that's provided with {a11ytables}.

### 2. Convert to a workbook

We can use `generate_workbook()` to convert our a11ytable to an [{openxlsx}](https://ycphs.github.io/openxlsx/) Workbook-class object. This function inserts into the sheets the information from our a11ytable and applies various mark-up and styles to meet the best practice guidance.
