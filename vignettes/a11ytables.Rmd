...
The function will return errors or warnings if anything is missing or seems odd.
...
So our a11ytable is basically just a dataframe with one row per sheet and one column per sheet element. In fact, it has class 'data.frame'/'tbl' along with 'a11ytable':

```{r check-class}
class(my_a11ytable)
```

For convenience, you can also check an object with `is_a11ytable()`.

It's also possible to convert a correctly-formatted data.frame directly to an a11ytable with `as_a11ytable()`.
...
While developing your output, you can open a temporary copy of the workbook with `openXL()`.
