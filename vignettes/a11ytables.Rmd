...
my_a11ytable <- new_a11ytable(
  ...
  sources = c(
    ...
    NA_character_,
    NA_character_, 
    "Motor Trend (1974)"  # will be added above the table
  ),
...
Note that it's also possible to convert a correctly-formatted pre-built data.frame directly to an a11ytable with `as_a11ytable()`, but `new_a11ytable()` is the preferred method.
...
You can apply some final tweaks to this file if the defaults don't quite meet your needs, though it's advisable to keep this to a minimum. At time of writing (v0.0.0.90014) you will need to address two accessibility requirements that are not yet covered by the package:
