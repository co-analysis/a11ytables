...
The tables below show how the features of the {a11ytables} package meet the GSS Best Practice and Impact Division's ['Making spreadsheets accessible: checklist of the basics'](https://gss.civilservice.gov.uk/policy-store/making-spreadsheets-accessible-a-brief-checklist-of-the-basics/).

This assessment is based on generating an unedited output from a typical {a11ytables} workflow: a `create_a11ytable()` passed to `generate_workbook()` and then written with `openxlsx::saveWorkbook()`.

There are several status values:

* 'included' means that {a11ytables} contains a feature to help meet the requirement
...
| :-- | :- | :- | :---- |
| Mark up tables | Yes | Included | Automatic table mark-up is provided by `generate_workbook()` |
| Give tables meaningful names | No | Included | Unique table names are generated from the `tab_titles` argument in `create_a11ytable()` |
| Remove merged cells, split cells and nested tables | Yes | Included | Merged/split cells and nested tables aren't supported by {a11ytables} |
...
| All tables should have one tagged header row | Yes | Included | R's data.frame class only allows for one header row, which must have unique names; the header row is tagged when marked-up as a table |
| Wrap text within cells | Yes | Included | `generate_workbook()` wraps content by default; there's a simple check in {a11ytables} to widen a column if it contains long strings |
...
| Description | Essential? | Status | Explanation |
| :-- | :- | :- | :---- |
| Do not use symbols or superscript to signpost to notes | Yes | Included/user’s responsibility | Notes are detected automatically as digits in square brackets; superscript is not supported by {a11ytables} |
...
| Put note text in a notes table on a notes worksheet | No | Included | `create_a11ytable()` expects the `sheet_type` 'notes' |
...
| Format text to make it accessible | No | Included | `generate_workbook()` auto-formats the text |
...
| Remove blank worksheets | Yes | Included | `create_a11ytable()` will error if expected argument content is missing |
| Use cells in column A wisely | Yes | Included | `generate_workbook()` inserts the sheet title, table count, notes statement, source statement and tables into column A by default  |
| Position tables against the left-hand edges of each sheet | Yes | Included | `generate_workbook()` inserts automatically the tables into column A |
| Avoid putting content below a table | No | Included | `generate_workbook()` adds information to rows above tables only |
