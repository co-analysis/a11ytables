...
* U = user's discretion (the user's responsibility)
* O = out of scope (not relevant to the purpose of the package)

### Tables

Status: Y = yes, P = partly, N = no, U = user's discretion, O = out of scope.
...
| 5 | All tables should have one tagged header row | Y | P |
| 6 | Wrap text within cells | Y | Y |
| 7 | Avoid adding filters and freeze panes | | Y |
| 8 | Only leave cells with no data empty in certain circumstances | Y | O |
...
| 2 | Use the word ‘note’ when Numbererring to footnotes | | Y |
...
| 10 | When using colour for emphasis check the contrast | Y | O |
